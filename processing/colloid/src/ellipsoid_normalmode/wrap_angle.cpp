 ...
 *
 * I found another thing to be noticed:
 * When deal with angle, since angle has 'periodic boundary condition', 
 * i.e. when angle > pi, then wrap out. So In my calculation, I should divide pi,
 * i.e.
 * delta \theta = (\theta - \theta_0) mod pi
 * So I need a method to deal with \theta_0, i.e.
 * N*\theta_0 = \sum (\theta_real)
 * where \theta_real is get as the method for periodic boundary condition.
 * It's easy to check whether wrapper is needed, since if one angle is near 0,
 * one is near pi, then the average will be far from them.
 */
		...
		bool Remove_Drift)
{
	...
	float ang, pi=acos(-1.0);
	int ipreviousAng= 2-size[0];
	char *notfirstframe = Calloc(char, total_gp); POINTER_NULL(notfirstframe);
		...
		for (i=cum_ptcl_frame[t]; i<cum_ptcl_frame[t+1]; i++)
		{
			...
			if ( p>=0 )
			{
				...
				*(pointer1+total_gp) += *(pointer+1);      // y
				// get angle real, i.e. remove the p.b.c
				if (notfirstframe[p]) // first frame
				{
					// check whether it's the first frame for that particle
					ang=*(pointer+2);
					*(pointer+2) = ang+
						(Nint((*(pointer+ipreviousAng)-ang)/pi))*pi;
				}
				else
					notfirstframe[p]=1;

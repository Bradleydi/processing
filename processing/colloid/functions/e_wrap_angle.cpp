...
int main (int argc, char *argv[])
{
	...
	else if (argc == 4)
	{
		...
		e_wrap_angle_normalmode(argv[1], -1.0, atof(argv[2]), atof(argv[3]), false);
		e_DOS(argv[1]);
		e_CDOS(argv[1]);
		e_PR(argv[1]);
	}
	else if (argc==5)
	{
		if (argv[4][0]=='-' && argv[4][1]=='r')
			e_wrap_angle_normalmode(argv[1], -1.0, atof(argv[2]), atof(argv[3]), true);
		else if (argv[4][0] != '-')
			e_wrap_angle_normalmode(argv[1], atof(argv[4]), \

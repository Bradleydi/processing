...
void DWfactor(const char *file, double a0, const int & dim, bool Remove_drift)
{
	...
	double *dx2=(double *) malloc( dim*total_t*sizeof(double) );
	POINTER_NULL(dx2);

	double *DW=(double *) malloc (total_p*sizeof(double));
	POINTER_NULL(DW);
	...
	for (p=0; p<total_p; p++)
	{
		...
		for (t=0; t<total_t; t++)
		{
			...
			if ( i != -1 )
			{
				...
				for (k=0; k<dim; k++)
				{
					dx2[count*dim+k] = (double) (*(pointer+k)) - mp[p*dim+k];
					dx2[count*dim+k] *= dx2[count*dim+k];
				}
				++count;
			}
		}
		DW[p]=pairwise(count*dim, dx2)/count;
	}
